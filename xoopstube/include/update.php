...
$ret[$i]   = true;
$query[$i] = sprintf(
    "ALTER TABLE " . $xoopsDB->prefix('xoopstube_videos')
    . " CHANGE urlrating vidrating TINYINT(1) NOT NULL default '0'"
);
$ret[$i]   = $ret[$i] && $xoopsDB->query($query[$i]);
$i++;
$ret[$i]   = true;
...
$query[$i] = sprintf(
    "ALTER TABLE " . $xoopsDB->prefix('xoopstube_videos')
    . " ADD COLUMN item_tag TEXT NOT NULL default '' AFTER keywords"
);
$ret[$i]   = $ret[$i] && $xoopsDB->query($query[$i]);
$i++;
$ret[$i]   = true;
...
$ret[$i]   = true;
$query[$i] = sprintf(
    "ALTER TABLE " . $xoopsDB->prefix('xoopstube_cat')
    . " ADD COLUMN client_id INT(5) NOT NULL default '0' AFTER weight"
);
$ret[$i]   = $ret[$i] && $xoopsDB->query($query[$i]);
$i++;
$ret[$i]   = true;
$query[$i] = sprintf(
    "ALTER TABLE " . $xoopsDB->prefix('xoopstube_cat')
    . " ADD COLUMN banner_id INT(5) NOT NULL default '0' AFTER client_id"
);
$ret[$i]   = $ret[$i] && $xoopsDB->query($query[$i]);
...
$ret[$i]   = true;
$query[$i] = sprintf(
    "ALTER TABLE " . $xoopsDB->prefix('xoopstube_mod')
    . " ADD COLUMN time VARCHAR(5) NOT NULL default '00:00' AFTER vidrating"
);
$ret[$i]   = $ret[$i] && $xoopsDB->query($query[$i]);
$i++;
$ret[$i]   = true;
...
$ret[$i]   = true;
$query[$i] = sprintf(
    "ALTER TABLE " . $xoopsDB->prefix('xoopstube_indexpage')
    . " ADD COLUMN lastvideosyn TINYINT(1) NOT NULL default '0' AFTER indexfooteralign"
);
$ret[$i]   = $ret[$i] && $xoopsDB->query($query[$i]);
$i++;
$ret[$i]   = true;
$query[$i] = sprintf(
    "ALTER TABLE " . $xoopsDB->prefix('xoopstube_indexpage')
    . " ADD COLUMN lastvideostotal VARCHAR(5) NOT NULL default '5' AFTER lastvideosyn"
);
$ret[$i]   = $ret[$i] && $xoopsDB->query($query[$i]);
