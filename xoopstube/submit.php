...
$xoopsTpl->assign(
    'xoops_module_header',
    '<link rel="stylesheet" type="text/css" href="' . xoopstube_url . '/xtubestyle.css" />'
);
...
// Notify of new link (anywhere) and new link in category
            ...
            if (true == xtubeCheckGroups($cid, 'XTubeAutoApp') || $approve == 1) {
                ...
                $sql     = "UPDATE " . $xoopsDB->prefix('xoopstube_videos')
                           . " SET cid=$cid, title='$title', vidid='$vidid', publisher='$publisher', updated='$updated', offline='$offline', description='$descriptionb', ipaddress='$ipaddress', notifypub='$notifypub', vidrating='$vidrating', time='$time', keywords='$keywords', item_tag='$item_tag', picurl='$picurl' WHERE lid =" . $lid;
                if (!$result = $xoopsDB->query($sql)) {
                    $_error = $xoopsDB->error() . " : " . $xoopsDB->errno();
                    XoopsErrorHandler_HandleError(E_USER_WARNING, $_error, __FILE__, __LINE__);
                }
...
// Show disclaimer
        if ($xoopsModuleConfig['showdisclaimer'] && !isset($_GET['agree']) && $approve == 0) {
            ...
            echo '<input type="button" onclick="location=\'submit.php?agree=1\'" class="formButton" value="'
...
// Video code form
        ...
        $videocode->setDescription('<br /><span style="font-size: small;">' . _MD_XTUBE_VIDEO_DLVIDIDDSC . '</span>');
...
//        $sform -> addElement( $editor, true );
        ...
        if (class_exists('XoopsFormEditor')) {
            ...
            $editor = new XoopsFormDhtmlTextArea('', 'descriptionb', $item->getVar(
                'descriptionb',
                'e'
            ), '100%', '100%');
            $optionsTrayNote->addElement($editor);
