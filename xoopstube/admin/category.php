...
//        $groups        = $groups;
    ...
    $indeximage_select->setExtra(
        "onchange='showImgSelected(\"image\", \"imgurl\", \"" . $xoopsModuleConfig['catimage'] . "\", \"\", \""
        . XOOPS_URL . "\")'"
    );
    ...
    if (!empty($imgurl)) {
        $indeximage_tray->addElement(
            new XoopsFormLabel('',
                "<br /><br /><img src='" . XOOPS_URL . "/" . $xoopsModuleConfig['catimage'] . "/" . $imgurl
...
//    $sform->addElement($editor, false);
    ...
    $optionsTrayNote = new XoopsFormElementTray(_AM_XTUBE_FCATEGORY_DESCRIPTION, '<br />');
    if (class_exists('XoopsFormEditor')) {
        ...
        $editor = new XoopsFormDhtmlTextArea('', 'description', $item->getVar(
            'description',
            'e'
        ), '100%', '100%');
        $optionsTrayNote->addElement($editor);
...
switch ($op) {
    case 'move':
        if (!isset($_POST['ok'])) {
            ...
            xoops_cp_header();
            //xtubeRenderAdminMenu(_AM_XTUBE_MCATEGORY);
    ...
    case 'addCat':
        ...
        if (!$cid) {
            ...
            $sql           = "UPDATE " . $xoopsDB->prefix('xoopstube_cat')
                             . " SET title ='$title', imgurl='$imgurl', pid =$pid, description='$descriptionb', spotlighthis='$spotlighthis' , spotlighttop='$spotlighttop', nohtml='$nohtml', nosmiley='$nosmiley', noxcodes='$noxcodes', noimages='$noimages', nobreak='$nobreak', weight='$weight', client_id='$client_id', banner_id='$banner_id' WHERE cid=" . intval(
                                 $cid
                             );
