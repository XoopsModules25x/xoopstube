...
switch (strtolower($op)) {
    ...
    default:
        $sql
            = 'SELECT indeximage, indexheading, indexheader, indexfooter, nohtml, nosmiley, noxcodes, noimages, nobreak, indexheaderalign, indexfooteralign, lastvideosyn, lastvideostotal FROM ' . $xoopsDB->prefix(
                'xoopstube_indexpage'
            );
        if (!$result = $xoopsDB->query($sql)) {
            XoopsErrorHandler_HandleError(E_USER_WARNING, $sql, __FILE__, __LINE__);
        ...
        list($indeximage, $indexheading, $indexheader, $indexfooter, $nohtml, $nosmiley, $noxcodes, $noimages, $nobreak, $indexheaderalign, $indexfooteralign, $lastvideosyn, $lastvideostotal) = $xoopsDB->fetchrow(
            $result
        );

        xoops_cp_header();
        //xtubeRenderAdminMenu( _AM_XTUBE_INDEXPAGE );
        ...
        $indeximage_select->setExtra(
            "onchange='showImgSelected(\"image\", \"indeximage\", \"" . $xoopsModuleConfig['mainimagedir']
            . "\", \"\", \"" . XOOPS_URL . "\")'"
        );
        ...
        if (!empty($indeximage)) {
            $indeximage_tray->addElement(
                new XoopsFormLabel('',
                    '<br /><br /><img src="' . XOOPS_URL . '/' . $xoopsModuleConfig['mainimagedir'] . '/' . $indeximage
...
//        $sform->addElement($editor, false);
        ...
        $optionsTrayNote = new XoopsFormElementTray(_AM_XTUBE_IPAGE_CHEADING, '<br />');
        if (class_exists('XoopsFormEditor')) {
            ...
            $editor = new XoopsFormDhtmlTextArea('', 'indexheader', $item->getVar(
                'indexheader',
                'e'
            ), '100%', '100%');
            $optionsTrayNote->addElement($editor);
        ...
        $headeralign_select->addOptionArray(
            array(
                 'left'   => _AM_XTUBE_IPAGE_CLEFT,
                 'right'  => _AM_XTUBE_IPAGE_CRIGHT,
                 'center' => _AM_XTUBE_IPAGE_CCENTER
            )
        );
        ...
        $sform->addElement(
            new XoopsFormRadioYN(_AM_XTUBE_IPAGE_SHOWLATEST, 'lastvideosyn', $lastvideosyn, ' ' . _YES . '',
                ' ' . _NO . '')
        );
        ...
        $lastvideostotalform->setDescription(
            '<span style="font-size: small;">' . _AM_XTUBE_IPAGE_LATESTTOTAL_DSC . '</span>'
        );
