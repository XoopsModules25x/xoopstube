...
switch (strtolower($op)) {
    case 'listmodreqshow':
        ...
        $sql       = 'SELECT modifysubmitter, requestid, lid, cid, title, vidid, submitter, publisher, vidsource, description, time, keywords, item_tag, picurl FROM '
                     . $GLOBALS['xoopsDB']->prefix('xoopstube_mod') . ' WHERE requestid=' . $requestid;
        $mod_array = $GLOBALS['xoopsDB']->fetchArray($GLOBALS['xoopsDB']->query($sql));
        unset($sql);

        $sql        = 'SELECT submitter, lid, cid, title, vidid, submitter, publisher, vidsource, description, time, keywords, item_tag, picurl FROM ' . $GLOBALS['xoopsDB']->prefix('xoopstube_videos')
        ...
        $submittername  = XoopstubeUtilities::xtubeGetLinkedUserNameFromId($orig_array['submitter']);
        ...
        $submittername  = XoopstubeUtilities::xtubeGetLinkedUserNameFromId($mod_array['modifysubmitter']);
        $submitteremail = $orig_user::getUnameFromId('email');
    ...
    default:
        ...
        if ($totalmodrequests > 0) {
            while (false !== ($video_arr = $GLOBALS['xoopsDB']->fetchArray($result))) {
                ...
                $title       = trim($path);
                $submitter   = XoopstubeUtilities::xtubeGetLinkedUserNameFromId($video_arr['modifysubmitter']);
                $requestdate = XoopstubeUtilities::xtubeGetTimestamp(formatTimestamp($video_arr['requestdate'], $GLOBALS['xoopsModuleConfig']['dateformatadmin']));
